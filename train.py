...
tf.flags.DEFINE_integer("ep_per_epoch",     10,  "Number of episodes per epochs")
tf.flags.DEFINE_float(  "learning_rate",    10e-5, "The learning rate")

### MACN conf
tf.flags.DEFINE_integer("batch_size",   2,  "Batch size (batch of episode)")
...
def main(args):
    ...
    def train_on_episode_batch(batch_images, batch_labels):
        _, _loss, _nb_err = sess.run([train_step, loss, nb_errors], feed_dict={macn.X_batch : batch_images, y : batch_labels})
        return _loss, _nb_err
        
    def test_on_episode_batch(batch_images, batch_labels):
        return sess.run([loss, nb_errors], feed_dict={macn.X_batch : batch_images, y : batch_labels})
